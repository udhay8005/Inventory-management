...
stock to the source slot. Tested via raw SQL UPDATE so we hit the DB guarantee
directly, mirroring wms_barcode/tests/test_audit_integrity.py.
"""
...
class TestRepairDamageQuantity(TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.product = cls.env["product.product"].create({"name": "Qty Integrity Product"})
